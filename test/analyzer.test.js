...
const semanticChecks = [
  ...
  ["function declaration and call", "playtime add(a: numba, b: numba): numba { bedtime a + b } gibberish(add(5, 7))"],
  ["Can look up variable in parent scope", "mine x = 1 playtime f(): numba { bedtime x } gibberish(f())"],
  ...
  ["print result of function call", "playtime add(a: numba, b: numba): numba { bedtime a + b } gibberish(add(2, 3))"],
  ["function with no parameters", "playtime getFive(): numba { bedtime 5 } gibberish(getFive())"],
  ["function with boolean parameter", "playtime not(x: squarehole): squarehole { bedtime gaagaa } gibberish(not(googoo))"],
  ["function with number parameter", "playtime square(x: numba): numba { bedtime x * x } gibberish(square(4))"],
  [
  ...
  ["function with void return", "playtime doSomething(): nada { bedtime }"],
  ["function with return value", "playtime getNumber(): numba { bedtime 42 } gibberish(getNumber())"],
...
const semanticErrors = [
  ...
  ["redeclaration of variable", "mine x = 1 mine x = 2", /Variable already declared/],
  ...
  ["non-boolean condition in if-else", "peekaboo 1 { gibberish(1) } nuhuh { gibberish(2) }", /Expected a boolean/],
  ["non-boolean condition in while", "wawawa 1 { gibberish(1) }", /Expected a boolean/],
  ["number variable as if condition", "mine x = 1 peekaboo x { gibberish(x) }", /Expected a boolean/],
  ["number variable as while condition", "mine x = 1 wawawa x { gibberish(x) }", /Expected a boolean/],
  ["type mismatch assigning boolean to number variable", "mine x = 1 x = gaagaa", /Type mismatch/],
  ["type mismatch assigning number to boolean variable", "mine x = gaagaa x = 1", /Type mismatch/],
  ["boolean in multiplication left operand", "gibberish(gaagaa * 1)", /Expected a number/],
  ["boolean in multiplication right operand", "gibberish(1 * gaagaa)", /Expected a number/],
  ...
  ["boolean in exponentiation exponent", "gibberish(2 ** gaagaa)", /Expected a number/],
  ...
  ["redeclaration in while body", "mine x = 1 wawawa gaagaa { mine x = 2 }", /Variable already declared/],
  ["redeclaration in if body", "mine x = 1 peekaboo gaagaa { mine x = 2 }", /Variable already declared/],
  ["undeclared variable in binary expression", "gibberish(x + 1)", /Undefined variable/],
  ["undeclared variable in comparison", "gibberish(x < 1)", /Undefined variable/],
  ["undeclared variable with multiple scopes", "mine x = 3 playtime f(x: numba): numba { bedtime y }", /Undefined variable/],
  ["wrong number of arguments in function call", "playtime f(): numba { bedtime 1 } gibberish(f(1))", /Expected 0 arguments/],
  [
    "type mismatch in function call argument",
    "playtime f(x: numba): numba { bedtime 1 } gibberish(f(gaagaa))",
    /Type mismatch in argument 1/,
  ],
  ["string and number addition", 'gibberish("hello" + 1)', /cannot concatenate number with string/],
  ["string and boolean addition", 'gibberish("hello" + gaagaa)', /cannot concatenate boolean with string/],
  ["string multiplication", 'gibberish("hello" * 3)', /is not supported for strings/],
  ["string division", 'gibberish("hello" / 3)', /is not supported for strings/],
  ["string modulo", 'gibberish("hello" % 3)', /is not supported for strings/],
  ["string exponentiation", 'gibberish("hello" ** 3)', /expected a number/i],
  ["string negation", 'gibberish(-"hello")', /expected a number/i],
  ["string subtraction", 'gibberish("hello" - 3)', /unsupported operator/i],
  ["string less than", 'gibberish("hello" < 3)', /Expected a number/i],
  ["random number generation with non-number bounds", "flippy(1, gaagaa)", /Expected a number/],
  ["random number generation with non-number bounds", "flippy(gaagaa, 10)", /Expected a number/],
